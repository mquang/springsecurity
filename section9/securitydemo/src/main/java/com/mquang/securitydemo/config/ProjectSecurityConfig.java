...
import com.mquang.securitydemo.filter.AuthoritiesLoggingAfterFilter;
import com.mquang.securitydemo.filter.CsrfCookieFilter;
import com.mquang.securitydemo.filter.RequestValidationBeforeFilter;
import com.mquang.securitydemo.filter.AuthoritiesLoggingAtFilter;
...
public class ProjectSecurityConfig {
	...
	SecurityFilterChain overrideDefaultSecurityFilterChain(HttpSecurity http) throws Exception {
		...
		.csrf((csrf) -> csrf.csrfTokenRequestHandler(requestHandler).ignoringRequestMatchers("/h2-console/**", "/contact", "/register")
				 ...
				 * 	+ ...
				 * 
				 * Spring Security is based on a chain of servlet filters. Each filter has a specific responsibility and 
				 * depending on the configuration, filters are added or removed. 
				 * 
				 * We can add our custom filters based on the need by 
				 * + implementing jakarta.servlet.Filter 
				 * or 
				 * + extending org.springframework.web.filter.GenericFilterBean (this is a handy superclass provides details about config parameters, init parameters, servlet context parameters that you have configured inside web.xml or inside your deployment descriptors.)
				 * or
				 * + extending org.springframework.web.filter.OncePerRequestFilter (override doFilterInternal)
				 */
